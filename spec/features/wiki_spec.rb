...
describe "Wikis" do
  ...
  describe "with an unsigned user" do
    ...
    describe "viewing individual wikis" do
      it "shows all the individual wiki's details" do
        ...
        #first(:link, "Wiki Title 1").click # not necessary for guests
    ...
    describe "creation" do
      ...
      #  login_as(my_user, scope: :user) # to sign in
      #  my_wiki
        visit(wikis_path)
        expect(page).to_not have_link("Create new wiki")
  ...
  end   # with an unsigned user



  describe "with user signed in as a standard user" do
    before { login_as(my_user, scope: :user) }
    ...
    describe "viewing individual wikis" do
      ...
      #  click_link("Wiki Title 1") # error ambiguous
      #  click_link("wiki-link-id") # did not work
      #  find('#wiki-link-id', visible: true).click # did not work
      #  click_link("Wiki Title 1", match: :first) # OK
        first(:link, "Wiki Title 1").click
        expect(current_path).to eq(wiki_path(my_wiki))
        expect(page).to have_content("Wiki Title 1")
        expect(page).to have_content("this is the wiki body")
    ...
    describe "creation" do
      it "saves the public wiki and show the wiki's detail page" do
        #login_as(my_user, user: my_user)
        #login_as(my_user, user: :user)
        #login_as(my_user, scope: :user) # I want to understand this login_as what is scope?
        visit(wikis_path)

        click_link("create-new-wiki") # uses an id
        # find("#create-new-wiki").click # OK; uses an id
        # first(:link, "Create new wiki").click # this is the best way; no id needed

        expect(current_path).to eq(new_wiki_path)
        fill_in("Title", with: "New Title")
        fill_in("Body", with: "New body")

        click_button("Create Wiki") # uses button
    ...
    describe "edit" do
      it "updates the public wiki and shows the wiki's updated details" do
        #login_as(my_user, scope: :user)
    ...
    describe "delete" do
      it "removes the user's wiki and shows the wiki list without the deleted wiki" do
        #login_as(my_user, user: my_user)
        my_wiki; my_wiki2
        visit(wiki_path(my_wiki))
  ...
  end   # with user signed in as a standard user
